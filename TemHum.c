...
static void uartPuts( const char* src, size_t size);
static void Seriel_output(char *str);
...
int main(){
    ...
    receive_val =  (ReceiveBuffer[0]) |  (ReceiveBuffer[1]) << 8;
    //int str = buffer[0];
...
while(1){
    ...
    UART_SendString("\n Temperature: ");
    UART_SendString(rx_char);

    ltoa(rel_H, rx_char, 10);
    UART_SendString("\n Humidity: ");
    UART_SendString(rx_char);
...
void init_i2c (void)
{
    ...
    masterSend2byte (EUSCI_B0_BASE,  register_int, enable_int);
        ...
        /*humidity_raw = (buffer[3] << 8) + buffer[2];
        rel_H = (humidity_raw/65536.0)*100;

        temperature_raw = (buffer[1] << 8) + buffer[0];
        temp_C = (temperature_raw/65536.0)*165-40; */
...
void USCIB0_ISR(void)
{
    ...
    switch(__even_in_range(UCB0IV, USCI_I2C_UCBIT9IFG))
    {
        ...
        case USCI_I2C_UCRXIFG0:     // RXIFG0
            // Get RX data
           receive_val = UCB0RXBUF;
          if (rxcount) {
              ReceiveBuffer[ReceiveIndex++] = receive_val;
              rxcount--;
